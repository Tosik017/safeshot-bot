...
### 10.1 Фикс CI на следующий день (2026-06-23)

Первый прогон `.github/workflows/ci.yml` упал: `ModuleNotFoundError: No module named 'bot'`/`'security'` на шаге "Run tests". Причина — `tests/` без `__init__.py`/`conftest.py`, а команда в workflow была `pytest -q` (голий консольний скрипт). Консольний скрипт **pytest не додає кореневу директорію репо в `sys.path`** — на відміну від `python -m pytest`, який робить це завжди (бо `-m` сам додає cwd). Локально баг не ловився — тести завжди запускались через `python -m pytest`. Виправлено в `f6d7010`: `run: python -m pytest -q`.

**Пастка на майбутнє:** кнопка «Re-run all jobs» в GitHub Actions перезапускає workflow-файл ТАКИМ, яким він був У ТОМУ КОМІТІ, а не останню версію з `main` — щоб перевірити фікс самого `.yml`, потрібен новий push, а не Re-run старого прогону. Це збило з пантелику при першій спробі перевірки фіксу.

CI зараз зелений (`f6d7010`, `success`). Render весь цей час продовжував працювати штатно — CI ніяк не пов'язаний з прод-деплоєм, лише з GitHub-репозиторієм.
